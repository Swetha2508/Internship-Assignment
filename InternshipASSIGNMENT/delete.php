<html>
    <head> <title>Super Admin - Delete</title>
    <meta name="UTF-8">
    <meta name="viewport" content="width=device-width,initial-scale=1.0">
    <style>
         body {
            background: linear-gradient(to right, #51A3A3, #75485E); /* Gradient Background */
  ...
  <?php
    ...
    if(isset($_POST['submit']))
    {
        ...
        $pwd = $_POST['pwd'];
        $role = $_POST['role'];
        $query = "DELETE FROM sadmin where email = '$email'  AND pwd = '$pwd'";
        if($db->query($query)==TRUE)
        {

             $Urole = strtoupper($role);
            echo "<script type='text/javascript'>alert('$Urole DATA Deleted Successfully. Thank You!!') </script>";
        }
        else
        {
              echo 'Error Occured';
